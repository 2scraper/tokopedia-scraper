...
page. If Tokopedia renders a reCAPTCHA challenge anywhere — account, sign-in
...
# of it is ported here, because Tokopedia has no such page.
#
# What Tokopedia does instead is answer NOTHING. Measured 2026-09-10 from a
# residential address: the connection is accepted, TLS completes, the HTTP/2
# stream opens and is then reset — no status code, no page, no form, no image
# to read. There is nothing for a solver to answer, and nothing even to
# detect in a page's content. The response to that is a different exit
# address, and product_parser.detect_page_state reports it as "blocked"
# rather than "challenge" precisely so no solve is attempted.
#
