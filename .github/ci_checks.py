...
# Contexts in which a 32-hex string is plainly not a key. The last two are
# this site's: every Tokopedia product image is served from its CDN under a
# 32-hex object name —
#
#     …/tos-maliva-i-<zone>/<32 hex characters>~tplv-…
#
# so the committed fixtures carry several, and without these entries the
# repo's own credential grep fails on its own test data. Kept as a
# CONTEXT allowlist rather than by loosening the pattern: a bare 32-hex
# string anywhere else still fails, which is the point of the check.
HEX32_ALLOWED = ("sha", "hash", "nonce", "example", "md5", "digest",
                 "checksum", "tokopedia-static.net", "tokopedia.net")
